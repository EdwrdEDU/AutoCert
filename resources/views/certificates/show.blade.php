...
<div class="space-y-6 min-w-[800px]">
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('certificates.index') }}" class="text-indigo-600 hover:text-indigo-800 font-medium">
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <p class="text-lg text-gray-900">{{ $certificate->template->name }}</p>
                <p class="text-sm text-gray-500">
                    Type: <span class="font-medium">{{ strtoupper($certificate->template->file_type) }}</span>
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-lg text-gray-900">
                    {{ $certificate->generated_at ? $certificate->generated_at->format('M d, Y g:i A') : '-' }}
                </p>
            </div>
                ...
                <p class="text-lg text-gray-900">
                    @if($certificate->isGenerated())
                        ...
                        <span class="text-sm text-gray-500 ml-2">
                            ({{ strtoupper(pathinfo($certificate->generated_pdf_path, PATHINFO_EXTENSION)) }})
                        </span>
                    @else
        ...
        <!-- Recipient Data -->
        @if($certificate->recipient_data)
            <div class="border-t border-gray-200 pt-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Certificate Data</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($certificate->recipient_data as $key => $value)
                        <div class="bg-gray-50 p-4 rounded-lg">
                            <label class="block text-sm font-medium text-gray-700 mb-1">{{ $key }}</label>
                            <p class="text-gray-900 break-words">{{ $value }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        @if($certificate->isGenerated())
            <div class="border-t border-gray-200 pt-6 flex gap-4" x-data="{ showDeleteModal: false }">
                <a href="{{ route('certificates.download', $certificate) }}" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium transition">
